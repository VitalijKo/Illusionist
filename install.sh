...
function print_status() {
	echo -e "${WHITE}[${BLUE}+${WHITE}] ${1}";
}

function print_success() {
	echo -e "${WHITE}[${GREEN}+${WHITE}] ${1}";
}

function print_fail() {
	echo -e "${WHITE}[${RED}+${WHITE}] ${1}";
}

function print_slowly() {
	echo -ne "${WHITE}[${BLUE}+${WHITE}] " && for (( i=0; i<${#1}; i++ )); do echo -ne "${1:$i:1}"; sleep ${2}; done; echo;
}

function check_cmd() {
	-v ${1} >/dev/null 2>&1 && return 0 || return 1;
}

function install_python_for_debian_based() {
	apt install python3 python3-dev libpython-dev python3-pip;
}

function install_libnetfilter() {
	apt install libnetfilter-queue -y;
